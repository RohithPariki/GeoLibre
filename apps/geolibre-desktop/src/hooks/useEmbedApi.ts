...
export function useEmbedApi(
  ...
  useEffect(() => {
    ...
    const runCommand = async (command: EmbedCommand): Promise<unknown> => {
      switch (command.type) {
        case "loadProject":
          if (!useAppStore.getState().deploymentCapabilities.has("project:edit")) throw new Error("Missing project:edit capability");
        ...
        case "highlightFeature":
          ...
          return;
        case "openTool":
          if (!useAppStore.getState().deploymentCapabilities.has("processing:run")) throw new Error("Missing processing:run capability");
        ...
        case "addLayer": {
          const state = useAppStore.getState();
          if (!state.deploymentCapabilities.has("data:add")) throw new Error("Missing data:add capability");
          const layer = buildEmbedLayer(command.spec, state.layers);
          state.addLayer(layer, command.spec.beforeId);
          return layer.id;
        }
        case "addData": {
          if (!useAppStore.getState().deploymentCapabilities.has("data:add")) throw new Error("Missing data:add capability");
          const abort = new AbortController();
        ...
        case "exportImage": {
          if (!useAppStore.getState().deploymentCapabilities.has("export:data")) throw new Error("Missing export:data capability");
